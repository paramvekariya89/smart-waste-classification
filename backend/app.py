...
# ------------------------------------------------------------------------------
app = Flask(__name__)
# Enable CORS for all routes and origins (allowing Vercel, localhost, etc.)
...
def health():
    """Returns backend and YOLOv11n model health status."""
    return jsonify({
...
def predict():
    """Receives image upload or base64 frame, executes YOLOv11n inference, and returns detection results."""
    global yolo_model
    ...
    # 1. Check standard multipart/form-data upload
...
@app.route("/api/sample/<category>", methods=["GET"])
def get_sample(category=None):
    """Returns a curated sample image from sample_images/ directory for instant testing."""
    if not os.path.exists(SAMPLES_DIR):
        return jsonify({"success": False, "error": "Sample images directory not found on server."}), 404

    all_files = [f for f in os.listdir(SAMPLES_DIR) if f.lower().endswith(('.jpg', '.png', '.jpeg'))]
    if not all_files:
        return jsonify({"success": False, "error": "No sample images found."}), 404

    if category:
        # Filter files by category name
        matched = [f for f in all_files if category.lower() in f.lower()]
        if matched:
    ...
    file_path = os.path.join(SAMPLES_DIR, selected)
...
if __name__ == "__main__":
    ...
    logger.info("Starting Smart Waste Classification Backend API on port %d...", port)
